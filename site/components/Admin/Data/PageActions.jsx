...
const PageActions = ({ params }) => {
  ...
  const deleteBlogPost = async () => {
    confirm({ description: `Are you sure you want to delete "${params.row.title}"  this page ?`, confirmationText: "Yes" })
      .then(async () => {
        dispatch({ type: "NOTIFY", payload: { loading: true } });
        await deleteData(`admin/page/${params.id}`, auth.token).then(res => {
  ...
  return (
    <>
      ...
      <Menu anchorEl={anchorEl} open={open} onClose={handleClose} onClick={handleClose} PaperProps={{ elevation: 0 }} transformOrigin={{ horizontal: 'right', vertical: 'top' }} anchorOrigin={{ horizontal: 'right', vertical: 'bottom' }}>
        ...
        <MenuItem sx={{ color: "success.main" }} onClick={() => { router.push(`/admin/edit/page/${params.id}`) }}>
          <ListItemIcon>
