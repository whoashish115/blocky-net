...
const NavigationActions = ({ params }) => {
    ...
    const deleteNavigation = async () => {
        confirm({ description: `Are you sure you want to delete "${params.row.name}"  this navigation ?`, confirmationText: "Yes" })
            .then(async () => {
                dispatch({ type: "NOTIFY", payload: { loading: true } });
                await deleteData(`admin/navigation/${params.id}`, auth.token).then(res => {
    ...
    return (
        <>
            ...
            <Menu anchorEl={anchorEl} open={open} onClose={handleClose} onClick={handleClose} PaperProps={{ elevation: 0 }} transformOrigin={{ horizontal: 'right', vertical: 'top' }} anchorOrigin={{ horizontal: 'right', vertical: 'bottom' }}>
                ...
                <MenuItem sx={{ color: "success.main" }} onClick={() => { router.push(`/admin/edit/navigation/${params.id}`) }}>
                    <ListItemIcon>
                ...
                <MenuItem sx={{ color: "error.main" }} onClick={deleteNavigation}>
                    <ListItemIcon>
